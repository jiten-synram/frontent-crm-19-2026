...
import { leadsAPI, authAPI, teamAPI } from '@/lib/api';
...
const INITIAL: Record<string, any> = {
  ...
  source:'call', category:'', supplement:'',
...
export default function NewLeadModal({ open, onClose, onCreated }: Props) {
  ...
  // ── Categories: admin → sab, sales → sirf assigned wali ──────
  ...
  useEffect(() => {
    ...
    if (isAdmin()) {
      // Admin ko saari categories dikhao
      setCategories(CATEGORIES);
      // Users bhi load karo assign ke liye
      authAPI.getUsers({ role: 'sales', is_active: 'true' })
        .then((d: any) => setUsers(d?.users || []))
        .catch(() => {});
    } else {
      // Sales user ko sirf uski assigned categories dikhao
      setCatLoading(true);
      authAPI.getMyCategories()
        .then((d: any) => {
          const myCats = d?.categories || [];
          // Agar koi category assign nahi hai to sab dikhao fallback
          setCategories(myCats.length ? myCats : CATEGORIES);
          // First category default set karo
          if (myCats.length > 0) {
            setForm(f => ({ ...f, category: myCats[0] }));
          }
        })
        .catch(() => {
          // Error pe sab dikhao
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res?.isDuplicate) {
        setDupWarn(`⚠ Duplicate: A lead with this phone already exists (${res.duplicateOf?.name}). Lead created anyway.`);
      }
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="form-input" value={form.city}
              onChange={(e) => set('city', e.target.value)} placeholder="e.g. Bhopal" />
          </div>
          ...
          {/* ── Category — filtered by role ── */}
          <div>
            <label className="form-label">
              Category <span className="text-red-500">*</span>
              {!isAdmin() && (
                <span className="text-[10px] text-gray-400 ml-1">(assigned categories)</span>
              )}
            </label>
            {catLoading ? (
              <div className="form-input flex items-center gap-2 text-gray-400 text-xs">
          ...
          <div>
            ...
            <select className="form-select" value={form.supplement}
          ...
          <div>
            ...
            <input className="form-input" value={form.product_name}
              ...
              placeholder="e.g. Kidney Care 3-Month Pack" />
          </div>
